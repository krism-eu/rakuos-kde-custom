...
# dnf5.real è il gestore reale della base RakuOS.
# Il fallback consente allo script di funzionare anche durante
# una build su una base che non ha ancora il wrapper RakuOS.
...
# I pacchetti KDE, firmware e driver già presenti nella base KDE
# non vengono reinstallati qui.
...
# Questi pacchetti devono essere assenti dall'immagine finale.)
...
# Rimozione di componenti e branding Fedora non desiderati.
rm -f \
...
# Il servizio può non essere presente in una base incompleta:
# in quel caso non blocchiamo la build.
